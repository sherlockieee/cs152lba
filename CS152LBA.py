...
class DPDP():
  ...
  def __init__(self, master):
    ...
    self.chatWindow = Label(master, text="Welcome to Minerva CovidBot Berlin", bd=1, bg='white', fg='black', width=100, height=8, justify="left")
...
def read_py(A: Atom, V: Atom, Y: Variable) -> bool:
  ...
  if isinstance(Y, Variable):
    ...
    response = simpledialog.askstring("Input", f'{str(A)} {str(V)}?', parent=root)
    Y.unify(response)
    return True
  else:
    return False
